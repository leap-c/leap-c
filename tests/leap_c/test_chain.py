...
from leap_c.examples.chain.utils import animate_chain_position_3D
...
def test_chain_policy_evaluation_works():
    ...
    for _ in range(100):
        ...
        sim_x.append(mpc.ocp_solver.get(1, "x"))

    sim_x = np.array(sim_x)

    animate_chain_position_3D(simX=sim_x, xPosFirstMass=np.zeros(3))
    plt.show()
