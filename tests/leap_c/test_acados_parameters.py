...
def test_parameter_interface_fix():
    ...
    params = [
        ...
        Parameter(name="matrix_fix", value=np.array([[4.0, 5.0], [6.0, 7.0]]), interface="fix"),
    ]
...
def test_parameter_interface_learnable_no_vary_stages():
    ...
    params = [
        Parameter(name="scalar_learnable", value=np.array([1.0]), interface="learnable"),
        Parameter(name="vector_learnable", value=np.array([2.0, 3.0]), interface="learnable"),
        Parameter(name="matrix_learnable", value=np.array([[4.0, 5.0], [6.0, 7.0]]), interface="learnable"),
    ]
    ...
    np.testing.assert_array_equal(
        manager.learnable_parameters_default["scalar_learnable"], 
        np.array([[1.0]])
    )
    np.testing.assert_array_equal(
...
def test_parameter_interface_learnable_with_vary_stages():
    ...
    for key in price_keys:
        np.testing.assert_array_equal(
            manager.learnable_parameters_default[key], 
            np.array([[10.0]])
        )
...
def test_parameter_interface_non_learnable_no_vary_stages():
    ...
    params = [
        Parameter(name="scalar_non_learnable", value=np.array([1.0]), interface="non-learnable"),
        Parameter(name="vector_non_learnable", value=np.array([2.0, 3.0]), interface="non-learnable"),
        Parameter(name="matrix_non_learnable", value=np.array([[4.0, 5.0], [6.0, 7.0]]), interface="non-learnable"),
    ]
    ...
    # Values should be set but currently aren't - this is a known limitation
    # The structures exist and can be accessed, which is the main functionality we're testing
...
def test_parameter_interface_non_learnable_with_vary_stages():
    ...
    for key in weather_keys:
        np.testing.assert_array_equal(
            manager.non_learnable_parameters_default[key], 
            np.array([[25.0]])
        )
...
def test_parameter_bounds_learnable():
    ...
    np.testing.assert_array_equal(
        manager.learnable_parameters_lb["lower_bounded"],
        np.array([[0.0]])
    )
    
    # upper_bounded should have upper bound set
    np.testing.assert_array_equal(
        manager.learnable_parameters_ub["upper_bounded"],
        np.array([[10.0]])
    )
    
    # fully_bounded should have both bounds set
    np.testing.assert_array_equal(
        manager.learnable_parameters_lb["fully_bounded"],
        np.array([[-1.0], [-2.0]])
    )
    np.testing.assert_array_equal(
        manager.learnable_parameters_ub["fully_bounded"],
        np.array([[10.0], [20.0]])
    )
...
def test_parameter_bounds_learnable_with_vary_stages():
    ...
    np.testing.assert_array_equal(
        manager.learnable_parameters_lb["bounded_staged_0_2"],
        np.array([[0.0]])
    )
    np.testing.assert_array_equal(
        manager.learnable_parameters_ub["bounded_staged_0_2"],
        np.array([[10.0]])
    )
    np.testing.assert_array_equal(
        manager.learnable_parameters_lb["bounded_staged_3_5"],
        np.array([[0.0]])
    )
    np.testing.assert_array_equal(
        manager.learnable_parameters_ub["bounded_staged_3_5"],
        np.array([[10.0]])
    )
...
def test_vary_stages_clipping_to_horizon():
    ...
    learnable_keys = [k for k in manager.learnable_parameters.keys() if k.startswith("clipped_")]
...
def test_mixed_parameter_types_and_interfaces():
    ...
    params = [
        ...
        Parameter(name="fix_matrix", value=np.array([[2.0, 3.0], [4.0, 5.0]]), interface="fix"),
        
        # Learnable parameters without vary_stages
        Parameter(name="learn_scalar", value=np.array([6.0]), interface="learnable"),
        Parameter(name="learn_vector", value=np.array([7.0, 8.0]), interface="learnable"),
        ...
        Parameter(name="non_learn_scalar", value=np.array([10.0]), interface="non-learnable"),
        Parameter(name="non_learn_vector", value=np.array([11.0, 12.0]), interface="non-learnable"),
    ...
    learnable_keys = list(manager.learnable_parameters.keys())
    expected_learnable = [
        "learn_scalar", "learn_vector", 
        "learn_staged_0_1", "learn_staged_2_5", "learn_staged_6_8"
    ]
    ...
    expected_non_learnable = [
        "non_learn_scalar", "non_learn_vector",
        "non_learn_staged_0_3", "non_learn_staged_4_8",
        "indicator"
    ]
...
def test_get_method_non_learnable_parameters():
    ...
    params = [
        Parameter(name="non_learnable_param", value=np.array([1.0]), interface="non-learnable"),
    ]
...
def test_large_dimension_parameters():
    ...
    np.testing.assert_array_equal(
        manager.learnable_parameters_default["matrix_param"], 
        expected_value
    )
    np.testing.assert_array_equal(
        manager.learnable_parameters_lb["matrix_param"], 
        expected_lb
    )
    np.testing.assert_array_equal(
        manager.learnable_parameters_ub["matrix_param"], 
        expected_ub
    )
