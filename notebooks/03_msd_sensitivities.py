...
parameter — via ``.backward()`` or via ``torch.autograd.functional.jacobian``,
exactly like any other PyTorch op.
...
def _(mo):
    ...
    This notebook shows two routes to the same gradients:
    ...
    Both give the exact gradient from the solver's KKT system, and the gradient
    arrives shaped like the parameter tensor you passed. It ends with an
    interactive tangent line that slides along the policy and value curves using
    gradients precomputed in a single batched solve.

    For the lower-level exact KKT sensitivity API (`sensitivity(ctx, ...)`) and
    when it is worth reaching for, see `07_advanced_sensitivities.py`.
    """)
    return
...
def _(mo):
    ...
    policy is locally *flat* in the parameters and $\partial u_0^{*}/\partial p$
    ...
    penalises the force more, so both the policy $u_0^{*}$ and the value $V$
    vary **smoothly** with it.

    We solve the whole sweep in **one batched call** and read back the value and
    the policy, then one backward pass over each recovers its gradient at every
...
@app.cell
def _(N_SWEEP, diff_mpc, np, torch):
    ...
    # One batched solve over the whole r-sweep, then autograd for the gradients.
    _x0_batch = x0_far.repeat(N_SWEEP, 1)
    _r_param = torch.tensor(r_sweep).reshape(-1, 1).requires_grad_(True)
    _, _u0_sweep, _, _, _value_sweep = diff_mpc(
        x0=_x0_batch, params={"r_diag_sqrt": _r_param}
    )

    u0_curve = _u0_sweep.detach().numpy().ravel()  # policy u₀*(r), shape (B,)
    v_curve = _value_sweep.detach().numpy().ravel()  # value  V(r),   shape (B,)

    # Batched autograd on the same solve. Each batch element depends only on its
    # own r_i, so differentiating the summed output recovers the per-point
    # gradients — one backward pass, no per-point loop.
    du0_dr_curve = (
        torch.autograd.grad(_u0_sweep.sum(), _r_param, retain_graph=True)[0].numpy().ravel()
    )
    dV_dr_curve = torch.autograd.grad(_value_sweep.sum(), _r_param)[0].numpy().ravel()
...
def _(
    ...
    grad_fig.suptitle(f"Analytic tangent at x0 = [0.05, 0], r_diag_sqrt = {r0:.3f}")
...
def _(dV_dr, du0_dr, mo, r0, u0_r0, v_r0):
    ...
    using gradients that were **all precomputed in one batched solve** (one
    backward pass per curve), so every tangent is exact where no constraint binds.
    Contrast this with the saturated case above, where the force sits on the bound
    and its gradient would collapse to ≈ 0.
