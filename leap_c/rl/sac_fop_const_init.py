...
@register_trainer("sac_fop_const", SACFOPBaseConfig())
class ConstInitStrategy(SACFOPTrainer):
    """This initialization strategy always uses the a constant initial guess (given by all zeros)."""

    def train_loop(self) -> Iterator[int]:
        is_terminated = is_truncated = True
        episode_return = episode_length = np.inf
        policy_state_const = self.init_policy_state()
            ...
            if (
                ...
                with torch.no_grad():
                    (
                        ...
                        param_prime,
                        mpc_stats_prime,
                    ) = self.pi(o_prime, policy_state_const)
