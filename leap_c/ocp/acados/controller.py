...
class AcadosController(ParameterizedController):
    ...
    def forward(
        ...
        """Passes obs, param, action, and ctx, as-is to the AcadosDiffMpcTorch object. Note that
        param is assumed to be the learnable parameters only, while the non-learnable parameters are
        ...
        ctx_out, u0, x, u, value = self.diff_mpc(obs, action, param, p_stagewise, ctx=ctx)
        return ctx_out, u0, value
