...
class AcadosDiffMpcTorch(torch.nn.Module):
    ...
    def forward(
        ...
        ``u0`` matches the input ``u0`` and ``value`` is the cost of the constrained trajectory
        ...
        ctx, u0, x, u, value = self.autograd_fun.apply(ctx, x0, u0, p_global, p_stagewise, None)
        u0 = u0.to(dtype=self.dtype)
        x = x.to(dtype=self.dtype)
        u = u.to(dtype=self.dtype)
        value = value.to(dtype=self.dtype)
        return ctx, u0, x, u, value  # type:ignore
