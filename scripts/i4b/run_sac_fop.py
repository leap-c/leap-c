...
@dataclass
class RunI4bSacFopConfig:
    trainer: SacFopTrainerConfig = field(default_factory=SacFopTrainerConfig)


def create_cfg(seed: int = 0) -> RunI4bSacFopConfig:
    cfg = RunI4bSacFopConfig()
    ...
    cfg.trainer.actor.extractor_name = "i4b"
    cfg.trainer.actor.distribution_name = "squashed_gaussian"
    cfg.trainer.actor.residual = True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SAC-FOP for i4b environment.")
    ...
    parser.add_argument(
        ...
        "--reuse-code",
        action="store_true",
        help="Reuse compiled acados code. Compiles on first run.",
    )
    parser.add_argument(
        "--reuse-code-dir",
        type=Path,
        default=None,
        help="Directory for compiled acados code (default: leap_c default path).",
    )
