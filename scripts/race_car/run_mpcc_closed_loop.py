"""Closed-loop smoke test: MpccPlanner driving RaceCarEnv for one lap.

Parallel to ``run_planner_closed_loop.py`` but uses the MPCC formulation. The env
is unchanged so trajectory / time-series plots are directly comparable.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-dir", type=Path, default=Path("output/race_car_mpcc_closed_loop"))
    ...
    p.add_argument(
        ...
        default="cartesian",
        help="MPCC frame variant.",
    )
    p.add_argument("--v-theta-max", type=float, default=5.0)
    return p.parse_args()
...
def main() -> None:
    ...
    planner_cfg = MpccPlannerConfig(frame=args.frame, v_theta_max=args.v_theta_max)
    planner = MpccPlanner(cfg=planner_cfg)
    ...
    print(
        f"Starting MPCC closed loop (frame={args.frame}). "
        f"dt={env_cfg.dt}s, N={planner_cfg.N_horizon}, v_theta_max={planner_cfg.v_theta_max}"
    )
    ...
    ax.set_ylabel("y [m]")
    ax.set_title(
        f"MPCC ({args.frame}) closed-loop lap ({n_steps} steps, {n_steps * env_cfg.dt:.2f}s)"
    )
    ...
    fig.suptitle(f"MPCC ({args.frame}) closed-loop states / controls")
    fig.tight_layout()
